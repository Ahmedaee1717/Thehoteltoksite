...
function EmailAdminApp() {
  ...
  const handleAddMember = async (memberEmail, role) => {
    if (!selectedMailbox) return;
    
    try {
      const response = await fetch(`/api/email/shared-mailboxes/${selectedMailbox.id}/members/add`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.success) {
        setSuccess(`✅ Member ${memberEmail} added successfully!`);
        setShowAddMemberModal(false);
        loadMailboxMembers(selectedMailbox.id);
      } else {
        setError(data.error || 'Failed to add member');
      }
    } catch (err) {
  ...
  const handleRemoveMember = async (memberEmail) => {
    ...
    try {
      const response = await fetch(`/api/email/shared-mailboxes/${selectedMailbox.id}/members/${encodeURIComponent(memberEmail)}`, {
        method: 'DELETE'
      });
  ...
  const handleViewMembers = (mailbox) => {
    setSelectedMailbox(mailbox);
    loadMailboxMembers(mailbox.id);
  };
